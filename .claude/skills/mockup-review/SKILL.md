...
2. **Launch** — run `/mockup-review` to open the gallery server.
3. **User rates** — user assigns ratings and notes per mockup and per component.
4. **Read feedback** — run `/mockup-selections` to load full context.
...
Never implement production UI changes before reading `/mockup-selections`. Never override an `approved` pattern without explicit user instruction.
